...
QANDA = {
    "Introduce your college?" : "In today's educational landscape, making campuses smarter is crucial for improving how colleges operate. Our project focuses on creating simple yet effective systems using a mix of hardware and software. These systems will help with things like tracking attendance, managing entrances, saving energy, and responding to emergencies. By using technologies like facial recognition and sensors, we aim to build a network of smart solutions that make college life easier and safer. We'll pay close attention to setting up hardware correctly, writing easy-to-understand code, and testing everything thoroughly to ensure it works well on campus.",
...
def check_for_relevant_question(prompt):
     prompt_ai = prompt.replace(AI_NAME.lower(), "")
     openai.api_key = OPENAI_API_KEY
     response = openai.ChatCompletion.create(
         model= "gpt-3.5-turbo",
         messages=[{"role" : "user", "content" : f"{prompt_ai}(Don’t justify your answers. Don’t give information not mentioned in the CONTEXT INFORMATION.Only give answers which are relevent to college if there is any question which is irrelevent then simply appologize."}])
